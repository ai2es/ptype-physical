...
print(df_ASOS.shape)
missing_ASOS = [datetime.strptime(x, '%Y%m%d').strftime('%m/%d/%Y') for x in missing_ASOS][idx_s:idx_e]
df_ASOS = df_ASOS[df_ASOS.obdate.isin(missing_ASOS)]
print(df_ASOS.shape)
...
df_ASOS['precip_count_byhr'] = list(duplicate_counts['obtime'])
