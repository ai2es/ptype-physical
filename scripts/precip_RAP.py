...
with open(f"../notebooks/missing_{save_dir}.pkl", "rb") as f:
    missing = pickle.load(f)

print(df.shape)
if idx_e == len(missing):
    missing = missing[idx_s:]
else:
    missing = missing[idx_s:idx_e]
missing = [datetime.strptime(x, '%Y%m%d').strftime('%m/%d/%Y') for x in missing]
